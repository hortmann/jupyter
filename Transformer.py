...
class Transformer(nn.Module):
    ...
    def __init__(self, model_name_or_path: str, max_seq_length: Optional[int] = None,
     ...
     ##########################   HORST   #####################################
    def add_tokenlist(self,new_tokens):    
        #print("Model",self.auto_model)
        t=self.tokenizer
        m=self.auto_model
        print('# of tokens BEFORE: ',len(t))
        #v1 = [tok for tok,index in t.get_vocab().items()]
        num_added_toks = t.add_tokens(new_tokens)
        print('We have added', num_added_toks, 'tokens')
        m.resize_token_embeddings(len(t))
        print('# of tokens AFTER: ',len(t))
        #v2 = [tok for tok,index in t.get_vocab().items()]
    ...
    def forward(self, features):
       ...
       # print("INPUT FEATURES OF TRANSFORMER:",features)
